...
class _ColoringPageState extends ConsumerState<ColoringPage> {
            ...
            child: Center(
              child: AspectRatio(
                ...
                child: GestureDetector(
                  onTapUp: (details) => _tap(details.localPosition, artwork),
                  child: CustomPaint(painter: ColoringPainter(artwork: artwork, selectedColorId: selectedColorId, completedRegionIds: completed)),
                ),
              ),
            ),
          ),
  ...
  void _tap(Offset localPosition, DemoArtwork artwork) {
    ...
    if (region == null || region.colorId != selectedColorId || completed.contains(region.id)) return;
    setState(() => completed = {...completed, region.id});
    _save();
  }
}
...
class ColoringPainter extends CustomPainter {
  ColoringPainter({required this.artwork, required this.selectedColorId, required this.completedRegionIds});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final region in artwork.regions) {
      ...
      canvas.drawRect(rect.deflate(1), Paint()..style = PaintingStyle.stroke..color = region.colorId == selectedColorId ? Colors.black87 : Colors.black26);
      if (!done) {
